"""Index options expiry: no index entry from the Thursday before to the third Friday's close.

Section 316 read the first Friday's payrolls release and found nothing.
The third Friday of each month is the other fixed date in the index
calendar: monthly equity options and, every quarter, the index futures
expire, and the hedges around the strikes pin prices into the close and
release them afterwards. A breakout on an index in those two days fights
dealer hedging rather than order flow.

  live       every gated signal
  candidate  refuse signals on US500, US30, US100, DE40, EU50, FR40,
             UK100, J225 and HK50 on the Thursday before and on the third
             Friday of each month
  diag       the same on the third Friday of March, June, September and
             December only (the quarterly expiries)
...
ARMS = {"live": None, "candidate": "monthly", "diag": "quarterly"}
STEP = np.timedelta64(7, 'D')


INDICES = {"US500", "US30", "US100", "DE40", "EU50", "FR40", "UK100", "J225", "HK50"}


def expiry_window(ts, quarterly):
    t = pd.DatetimeIndex(ts)
    third_friday = (t.dayofweek == 4) & (t.day >= 15) & (t.day <= 21)
    if quarterly:
        return third_friday & np.isin(t.month, [3, 6, 9, 12])
    thursday_before = (t.dayofweek == 3) & (t.day >= 14) & (t.day <= 20)
    return third_friday | thursday_before


def signals(frames, atr_floor, meta, kind):
    out = []
    for pair, df in frames.items():
        n = len(df); ts = df["timestamp"].values
        O = df["open"].values; H = df["high"].values
        L = df["low"].values; C = df["close"].values
        blackout = expiry_window(ts, kind == "quarterly") if kind and pair in INDICES else None
        for s in STRATS:
            for x in get_strategy(s)(df, {}):
                ...
                if blackout is not None and blackout[x.index]: continue
...
async def main():
    ...
    for arm, kind in ARMS.items():
        sig = signals(frames, atr_floor, meta, kind)
