...
def _bot_status(label: str, pid_file: str, log_file: str) -> dict:
    ...
    last_hb = open_pos = notional = regime = last_ts = None
    holiday = False
    try:
        with open(os.path.join(_ROOT, log_file), encoding="utf-8",
                  errors="replace") as f:
            lines = f.read().replace("\x00", "").splitlines()
    except OSError:
        lines = []
    for offset, line in enumerate(reversed(lines)):
        if offset < 8 and "BANK HOLIDAY" in line:
            holiday = True
        if last_ts is None:
        ...
        if last_hb is None:
            ...
            if m:
                last_hb, open_pos = m.group(1), int(m.group(2))
        if notional is None:
        ...
        if regime is None:
            ...
            if m:
                regime = m.group(1)
        if all((last_ts, last_hb, notional, regime)):
            break
    ...
    if last_ts:
        try:
            ts = datetime.strptime(last_ts, "%Y-%m-%d %H:%M:%S").replace(
                tzinfo=timezone.utc)
            age_min = (now - ts).total_seconds() / 60
        except ValueError:
            pass

    # Derive a state + colour.
    if not alive:
        ...
        state, tone = f"Heartbeat alt ({age_min:.0f} min)", "warn"
    else:
        state, tone = "läuft", "ok"
    return {
        "label": label, "alive": alive, "pid": pid, "state": state,
        "tone": tone, "last_hb": last_hb, "open_pos": open_pos,
        "notional": notional, "regime": regime, "age_min": age_min,
    }
