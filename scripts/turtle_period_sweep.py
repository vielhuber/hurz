"""Channel-length sweep for turtle_breakout on the cost-charging walk-forward simulator.

Periods 20 / 55 / 110 against the live 55 at the live 2-ATR stop on the
ten unblocked instruments. DAYS_FROM / DAYS_TO select the history
window. See docs/EDGE_FINDINGS.md section 63.
"""
...
async def main():
    ...
    res={P:[] for P in PERIODS}; strat=get_strategy("turtle_breakout")
    try:
        for pair in PAIRS:
            bars=None
        ...
        await p.disconnect()
    print(f"\n===== TURTLE PERIOD SWEEP (stop 2.0) ({DAYS_FROM}-{DAYS_TO} d) — diff vs period 55 (live), Welch t =====")
    nb,mb,sb=stats(res[55])
    print(f"{'period':>7}{'n':>6}{'E[R]':>9}{'t0':>7}{'win%':>7}{'sumR':>8}{'diff':>9}{'t_diff':>8}")
    for P in PERIODS:
        n,mu,se=stats(res[P]); d=mu-mb; t=d/np.sqrt(se**2+sb**2) if P!=55 else 0.0
