"""Notional-cap sweep at the live 2-ATR stop with the live sizing function.

Caps of 250 / 300 / 400 USD on the ten core instruments: how often the cap
binds and what target risk it removes. DAYS_FROM / DAYS_TO select the
history window. See docs/EDGE_FINDINGS.md section 81.
"""
...
def run(df, entries, pair, CAP, cons):
    ...
    for e,d in sorted(entries):
        ...
        sz=calculate_position_size(entry_price=entry, stop_loss=sl, target_risk=DEFAULT_TARGET_RISK_USD, notional_cap=CAP, min_size=cons.min_size, size_increment=cons.size_increment)
        r=None
...
async def main():
    ...
    try:
        for pair in PAIRS:
            ...
            res[pair]={c:[] for c in CAPS}; res[pair]["cons"]=cons
            for s in STRATS:
                st=get_strategy(s)
                for k in range(SEG):
                    lo_=k*seg; hi=(k+1)*seg if k<SEG-1 else n
                    sdf=df.iloc[lo_:hi].reset_index(drop=True); sg=[(x.index,x.direction) for x in st(sdf,{})]
                    for sa in CAPS: res[pair][sa].extend(run(sdf,sg,pair,sa,cons))
            print(f"{pair} bars={n} min_size={cons.min_size} step={cons.size_increment}",flush=True)
            await asyncio.sleep(0.6)
    finally:
        await p.disconnect()
    print(f"\n===== NOTIONAL CAP SWEEP (stop 2 ATR) ({DAYS_FROM}-{DAYS_TO} d) =====")
    print(f"{'pair':<10}{'cap':>6}{'n':>6}{'skip%':>7}{'risk$':>7}{'stop%':>7}{'E_all':>8}{'E_kept':>8}{'E_skip':>8}{'E$/trade':>9}")
    tot={c:[] for c in CAPS}
    for pair in res:
        for sa in CAPS:
            ...
            kept=r[~sk]; skip=r[sk]
            print(f"{pair:<10}{sa:>6.0f}{len(r):>6}{sk.mean()*100:>7.1f}{rk[~sk].mean() if (~sk).any() else 0:>7.2f}{sp.mean():>7.2f}{r.mean():>+8.4f}{kept.mean() if len(kept) else 0:>+8.4f}{skip.mean() if len(skip) else 0:>+8.4f}{(kept*rk[~sk]).mean() if len(kept) else 0:>+9.3f}")
    for sa in CAPS:
        a=tot[sa]; r=np.array([x[0] for x in a]); sk=np.array([x[1] for x in a]); rk=np.array([x[2] for x in a])
        kept=r[~sk]
        print(f"{'POOLED':<10}{sa:>6.0f}{len(r):>6}{sk.mean()*100:>7.1f}{rk[~sk].mean():>7.2f}{'':>7}{r.mean():>+8.4f}{kept.mean():>+8.4f}{r[sk].mean() if sk.any() else 0:>+8.4f}{(kept*rk[~sk]).mean():>+9.3f}  usd/yr(kept)={(kept*rk[~sk]).sum():+.1f}")
asyncio.run(main())
