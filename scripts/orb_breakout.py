"""Opening-range breakout on the cost-charging walk-forward simulator.

The cash-open hour bar of each index is the range; the first close beyond
it within six bars of the same day enters in that direction, one trade
per day. Count-matched random entries and the live donchian_breakout on
the same bars are the comparisons. DAYS_FROM / DAYS_TO select the history
window. See docs/EDGE_FINDINGS.md section 56.
"""
...
OPEN_HOUR = {"DE40":7, "US500":13, "US30":13, "US100":13}
...
def orb_entries(df, open_hour):
    ts=df["timestamp"]; H=df["high"].values; L=df["low"].values; C=df["close"].values; out=[]
    for i in range(60,len(df)):
        t=ts.iloc[i]
        if t.hour!=open_hour or t.weekday()>4: continue
        hi,lo=H[i],L[i]
        for j in range(1,WINDOW+1):
            if i+j>=len(df): break
            if ts.iloc[i+j].date()!=t.date(): break
            if C[i+j]>hi: out.append((i+j,1)); break
            if C[i+j]<lo: out.append((i+j,-1)); break
    return out
...
async def main():
    ...
    try:
        ...
        await p.disconnect()
    print(f"\n===== OPENING-RANGE BREAKOUT ({DAYS_FROM}-{DAYS_TO} d) =====")
