"""The momentum strategy's EMA pair, swept for the first time.

`momentum` enters when EMA(10) crosses EMA(30) with the 10-bar rate of
change behind it. Section 305 moved the rate-of-change floor and found
that a stricter floor only removes the strategy; the two averages that
decide *when* the cross fires were never moved. A slower pair crosses
later and less often, on moves that have already lasted; a faster pair
crosses early and often.

  live       EMA(10) / EMA(30)
  candidate  EMA(20) / EMA(60)
  diag       EMA(5) / EMA(15)

Donchian and turtle, the rate-of-change floor, the stop, the target, the
leash, the sizes and every cap are unchanged. Weekly re-ranking, open
...
ARMS = {"live": (10, 30), "candidate": (20, 60), "diag": (5, 15)}
STEP = np.timedelta64(7, 'D')


def signals(frames, atr_floor, meta, spans):
    out = []
    for pair, df in frames.items():
        ...
        tuned = df.copy()
        tuned["ema_fast"] = tuned["close"].ewm(span=spans[0], adjust=False).mean()
        tuned["ema_slow"] = tuned["close"].ewm(span=spans[1], adjust=False).mean()
        for s in STRATS:
            for x in get_strategy(s)(tuned if s == "momentum" else df, {}):
...
async def main():
    ...
    for arm, spans in ARMS.items():
        sig = signals(frames, atr_floor, meta, spans)
        ...
        print(f"{arm:<10} signals {len(sig)}, trades {len(booked)}, "
              ...
              f"momentum {sum(t['strat'] == 'momentum' for t in booked)}", flush=True)
