"""A stagnation exit: out when the position stops making new extremes.

The leash exits every position after 24 bars, whatever it has been doing;
sections 198, 228, 250 and the runs of 2026-09-13 measured its length and
its split by open result. None of them read the shape of the move: a
position that printed its best price twelve bars ago and has gone nowhere
since holds a slot the book cannot use elsewhere, and trend following says
a move that has stopped extending is over.

  live       stop, target at 1.5 R, leash at 24 bars
  candidate  the same, plus an exit at the close of the bar 12 bars after
             the last new favourable extreme
  diag       the same at 8 bars

The stop, the target, the leash, the sizes and every cap are unchanged;
the candidate can only close a position earlier. Weekly re-ranking, open
positions and cooldowns carried across ranking boundaries (section 284).
...
ARMS = {"live": None, "candidate": 12, "diag": 8}
STEP = np.timedelta64(7, 'D')


def book(O, H, L, C, e, d, entry, stop_d, cost_r, n, stale):
    """`efficiency_weighted_selection.book` plus the stagnation exit."""
    ...
    for b in range(e + 1, e + HOLD + 1):
        ...
        progress = (favor - entry) * d
        if progress > best: best = progress; last_new = b
        elif stale is not None and b - last_new >= stale:
            return (float(C[b]) - entry) * d / stop_d - cost_r, b
