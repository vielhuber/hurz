"""The year-end holiday weeks as a blackout: no entries from 20 December to 3 January.

The calendar filters measured so far read the weekday, the turn of the
month, bank holidays, the payrolls Friday (316) and the index expiry
(317). The two weeks around the year end were never read as a block:
desks are thin, spreads widen, and a breakout in an empty market is more
often a single order than a move. The bank-holiday guard covers the
days themselves, not the weeks between them.

  live       every gated signal
  candidate  refuse signals from 20 December to 3 January inclusive
  diag       the same, and also every signal in August
...
ARMS = {"live": None, "candidate": "year_end", "diag": "year_end_august"}
STEP = np.timedelta64(7, 'D')


def thin_season(ts, august):
    t = pd.DatetimeIndex(ts)
    year_end = ((t.month == 12) & (t.day >= 20)) | ((t.month == 1) & (t.day <= 3))
    return year_end | (t.month == 8) if august else year_end
...
def signals(frames, atr_floor, meta, kind):
    ...
    for pair, df in frames.items():
        ...
        blackout = thin_season(ts, kind == "year_end_august") if kind else None
        for s in STRATS:
