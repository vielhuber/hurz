"""Range exhaustion: no entry once the day has already travelled its usual range.

The gates read ADX, volatility level, the broken level, the day's
direction (section 306) and the extension from EMA20 (run 40), never how
much of an ordinary day's range has already been used. A breakout on a
day whose high-low range already exceeds the average of the last twenty
days has less room left inside a normal session than one on a quiet day.

  live       every gated signal
  candidate  refuse a signal when the range of its UTC day so far (up to
             and including the signal bar) exceeds 1.5 x the mean range
             of the twenty previous UTC days
  diag       the same at 1.0 x
...
ARMS = {"live": None, "candidate": 1.5, "diag": 1.0}
STEP = np.timedelta64(7, 'D')


def range_used(ts, H, L, days_back=20):
    days = ts.astype('datetime64[D]')
    first = np.r_[True, days[1:] != days[:-1]]
    day_id = np.cumsum(first) - 1
    hi = pd.Series(H).groupby(day_id).cummax().values
    lo = pd.Series(L).groupby(day_id).cummin().values
    full = pd.Series(H).groupby(day_id).max().values - pd.Series(L).groupby(day_id).min().values
    avg = pd.Series(full).shift(1).rolling(days_back).mean().values
    return (hi - lo) / avg[day_id]


def signals(frames, atr_floor, meta, limit):
    out = []
    for pair, df in frames.items():
        n = len(df); ts = df["timestamp"].values
        O = df["open"].values; H = df["high"].values
        L = df["low"].values; C = df["close"].values
        used = range_used(ts, H, L) if limit else None
        for s in STRATS:
            for x in get_strategy(s)(df, {}):
                ...
                if used is not None and np.isfinite(used[x.index]) and used[x.index] > limit: continue
...
async def main():
    ...
    for arm, limit in ARMS.items():
        sig = signals(frames, atr_floor, meta, limit)
