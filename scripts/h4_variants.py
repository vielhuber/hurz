"""4h strategy variants at the live 2-ATR stop on the cost-charging walk-forward simulator.

Signals, router-passed signals and count-matched random entries on 4h
bars for ten instruments. DAYS_FROM / DAYS_TO select the history window.
See docs/EDGE_FINDINGS.md section 72.
"""
...
STRATS=["donchian_breakout","turtle_breakout"]
    ...
    try:
        for pair in PAIRS:
            ...
            for attempt in range(4):
                try:
                    bars=await p.fetch_history(pair, from_ts=datetime.now(timezone.utc)-timedelta(days=DAYS_FROM), to_ts=datetime.now(timezone.utc)-timedelta(days=DAYS_TO), resolution="4h"); break
        ...
        await p.disconnect()
    print(f"\n===== 4H VARIANTS AT 2-ATR STOP ({DAYS_FROM}-{DAYS_TO} d) =====")
