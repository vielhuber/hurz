...
case "$cmd" in
  ...
  stop)
    ...
    # watchdog PID leaves that python orphaned (reparented to init) and
    # still trading the account — which previously left zombie sessions
    # piling up across restarts. Collect and kill the whole descendant
    # tree (children first, then the watchdog).
    collect_tree() {
      local p="$1" k
