"""The stop floor as a parameter: 1.05 % (live) against 1.5, 2 and 3 % of price.

Section 135 established that the 1.05 % floor is the project's own
wide-stop setting, not the venue's rule, and that it beats the designed
2-ATR stop on both samples. Every earlier sweep varied the ATR multiple
above a fixed floor; the floor itself was never varied. Replays the three
live 1h trend strategies on the router-passed path with the live widening
rule, gap-aware stop booking and the commodity short block over all 26
tradeable instruments at 2 ATR under floors of 1.05, 1.5, 2 and 3 % of
price. DAYS_FROM / DAYS_TO select the window; DUMP stores per-variant
results. See docs/EDGE_FINDINGS.md section 136.
"""
...
VARIANTS=[("live: floor 1.05 %",0.0105,2.0),("floor 1.5 %",0.015,2.0),("floor 2.0 %",0.020,2.0),("floor 3.0 %",0.030,2.0)]
...
async def main():
    ...
    try:
        for pair in PAIRS:
            ...
            print(f"{pair} bars={n} live trades={len(per_pair[pair][0])} floor-1.5 trades={len(per_pair[pair][1])}",flush=True)
    finally:
        await p.disconnect()
    if DUMP:
    ...
    print(f"\n===== STOP FLOOR SWEEP at 2 ATR, router-passed ({DAYS_FROM}-{DAYS_TO} d) =====")
    table("ALL", allres, skips)
    for s in STRATS: table(s, per_s[s], {i:0 for i in allres})
    print("\n--- per instrument: live E[R] / floor 1.5 % E[R] / n live / n 1.5 %")
