"""A profit lock: once a position has run +1 R in favour, its stop moves to +0.5 R.

Run 3 of 2026-09-07 moved the stop to entry on distance at the old 1-ATR
stop, section 314 armed break-even on time at the live floor. Both only
scratched: the stop went to the entry, and the scratches cost the book the
positions that came back and ran on. A lock half-way between the entry and
the trigger keeps half a unit of room below the running extreme, so a
winner that pulls back half an R survives, and one that turns round
entirely still books +0.5 R instead of the full -1 R leg.

  live       stop, target at 1.5 R, leash at 24 bars
  candidate  the same; once a bar's extreme reaches +1.0 R, the stop
             moves to +0.5 R from the next bar on
  diag       the same at +0.75 R, locking +0.25 R

A lock is armed only after the bar's stop and target checks, so a bar that
touches both barriers still books against the old stop. A stop touched
after arming books the locked level minus costs; a gap through it books
the gap. Target, leash, sizes and every cap are unchanged. Weekly
...
ARMS = {"live": None, "candidate": (1.0, 0.5), "diag": (0.75, 0.25)}
STEP = np.timedelta64(7, 'D')


def book(O, H, L, C, e, d, entry, stop_d, cost_r, n, lock):
    """`efficiency_weighted_selection.book` plus the profit lock."""
    sl = entry - d * stop_d; tp = entry + d * RR * stop_d
    for b in range(e + 1, e + HOLD + 1):
        if b >= n: break
        gap = (O[b] - entry) * d
        if gap <= (sl - entry) * d: return gap / stop_d - cost_r, b
        ...
        if lock is not None and (favor - entry) * d >= lock[0] * stop_d:
            sl = entry + d * max((sl - entry) * d, lock[1] * stop_d)
    if e + HOLD < n: return (float(C[e + HOLD]) - entry) * d / stop_d - cost_r, e + HOLD
    return None, None


def signals(frames, atr_floor, meta, lock):
    out = []
    for pair, df in frames.items():
        ...
        for s in STRATS:
            for x in get_strategy(s)(df, {}):
                ...
                r, xb = book(O, H, L, C, x.index, x.direction, entry, stop_d, cost_r, n, lock)
...
async def main():
    ...
    for arm, lock in ARMS.items():
        sig = signals(frames, atr_floor, meta, lock)
