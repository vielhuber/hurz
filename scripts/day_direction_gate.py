"""The day's direction as a gate: no long below the day's open, no short above.

The gates read ADX, volatility, the broken level and the calendar, never
where the signal stands in its own trading day. A breakout long on a day
that still trades below its opening price runs against the session's net
flow; one above it runs with it.

  live       every gated signal
  candidate  refuse a long whose signal close is below the open of the
             first bar of its UTC day (a short: above it)
  diag       the same against the previous UTC day's last close
...
ARMS = {"live": None, "candidate": "open", "diag": "prev_close"}
STEP = np.timedelta64(7, 'D')


def day_anchors(ts, O, C):
    days = ts.astype('datetime64[D]')
    first = np.r_[True, days[1:] != days[:-1]]
    idx = np.maximum.accumulate(np.where(first, np.arange(len(ts)), 0))
    today_open = O[idx]
    prev_close = np.where(idx > 0, C[np.maximum(idx - 1, 0)], np.nan)
    return {"open": today_open, "prev_close": prev_close}


def signals(frames, atr_floor, meta, anchor):
    out = []
    for pair, df in frames.items():
        n = len(df); ts = df["timestamp"].values
        O = df["open"].values; H = df["high"].values
        L = df["low"].values; C = df["close"].values
        ref = day_anchors(ts, O, C)[anchor] if anchor else None
        for s in STRATS:
            for x in get_strategy(s)(df, {}):
                ...
                if ref is not None and np.isfinite(ref[x.index]) and (C[x.index] - ref[x.index]) * x.direction < 0: continue
...
async def main():
    ...
    results = {}
    for arm, anchor in ARMS.items():
        sig = signals(frames, atr_floor, meta, anchor)
