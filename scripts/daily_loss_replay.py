"""Daily-loss guard replay on the one-position-per-instrument timeline.

Blocks entries for the rest of a UTC day once closed R that day reaches
-limit, at limits of 3 / 6 / 9 R, on the cost-charging simulator at the
live 2-ATR stop. DAYS_FROM / DAYS_TO select the history window. See
docs/EDGE_FINDINGS.md section 84.
"""
...
async def main():
    ...
    import collections
    print(f"\n===== DAILY-LOSS GUARD REPLAY, ONE POSITION PER INSTRUMENT ({DAYS_FROM}-{DAYS_TO} d) — limit in R per UTC day, pooled book =====")
    base=sum(t[0] for t in trades); print(f"no guard: n={len(trades)} sumR={base:+.1f} E[R]={base/len(trades):+.4f}")
    for lim in LIMITS:
        closed_by_day=collections.defaultdict(float); blocked=[]; kept=[]; days_hit=set()
        # process in entry order; a trade's loss is booked at its exit day
        events=[]
        for r,c,et,xt in trades: events.append((et,"entry",r,xt))
        for et,_,r,xt in sorted(events):
            d=et.date()
            if closed_by_day[d] <= -lim: blocked.append(r); days_hit.add(d); continue
            kept.append(r); closed_by_day[xt.date()]+=r
        print(f"limit {lim:>4.1f}R: kept n={len(kept)} sumR={sum(kept):+.1f} | blocked n={len(blocked)} sumR_blocked={sum(blocked):+.1f} E[R]_blocked={(sum(blocked)/len(blocked)) if blocked else 0:+.4f} | days hit={len(days_hit)} | delta vs no guard={sum(kept)-base:+.1f}")
asyncio.run(main())
