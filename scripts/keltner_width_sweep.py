"""Band-width sweep for keltner_breakout on the cost-charging walk-forward simulator.

ATR multiples 1.0 / 1.5 / 2.0 / 3.0 against the live 2.0 on the ten
unblocked instruments. DAYS_FROM / DAYS_TO select the history window.
See docs/EDGE_FINDINGS.md section 59.
"""
...
async def main():
    ...
    res={P:[] for P in PERIODS}; strat=get_strategy("keltner_breakout")
    try:
        for pair in PAIRS:
            bars=None
            ...
            for k in range(SEG):
                ...
                for P in PERIODS:
                    res[P].extend(run_trades(sdf,[(x.index,x.direction) for x in strat(sdf,{"atr_mult":P})],pair))
            await asyncio.sleep(0.6)
    finally:
        await p.disconnect()
    print(f"\n===== KELTNER ATR_MULT SWEEP ({DAYS_FROM}-{DAYS_TO} d) — diff vs atr_mult 2.0 (live), Welch t =====")
    nb,mb,sb=stats(res[2.0])
    print(f"{'mult':>7}{'n':>6}{'E[R]':>9}{'t0':>7}{'win%':>7}{'sumR':>8}{'diff':>9}{'t_diff':>8}")
    for P in PERIODS:
        n,mu,se=stats(res[P]); d=mu-mb; t=d/np.sqrt(se**2+sb**2) if P!=2.0 else 0.0
        print(f"{P:>7.1f}{n:>6}{mu:>+9.4f}{mu/se:>+7.2f}{(np.asarray(res[P])>0).mean()*100:>7.1f}{mu*n:>+8.1f}{d:>+9.4f}{t:>+8.2f}")
asyncio.run(main())
