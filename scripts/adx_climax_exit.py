"""Closing an open position when its ADX reaches the router's ceiling.

The router refuses a new trend entry at ADX >= 50 (the ceiling, built in
on three samples): a trend that strong has mostly run. An open position
is not asked the same question. Run 20 of 2026-09-07 closed trades when
ADX fell *below* 15/20/25 and lost; the climax side — banking a position
once its own instrument's trend reads overextended — was never measured.

  live       stop, target or the 24th bar, as today
  candidate  additionally closed at the close of the first bar after entry
             whose ADX(14) is >= 50
  diag       the same at >= 60

No extra cost charged. Stop, target, size and every guard unchanged; the
selector ranks on each arm's own booking.
...
ARMS = {"live": None, "candidate": 50.0, "diag": 60.0}


def book_climax(O, H, L, C, X, e, d, entry, stop_d, cost_r, n, ceiling):
    ...
    for b in range(e + 1, e + HOLD + 1):
        ...
        if ceiling is not None and b < e + HOLD and np.isfinite(X[b]) and X[b] >= ceiling:
            return (float(C[b]) - entry) * d / stop_d - cost_r, b, True
    if e + HOLD < n: return (float(C[e + HOLD]) - entry) * d / stop_d - cost_r, e + HOLD, False
    return None, None, False


def signals(frames, atr_floor, meta, ceiling):
    out = []
    for pair, df in frames.items():
        n = len(df); ts = df["timestamp"].values
        O = df["open"].values; H = df["high"].values
        L = df["low"].values; C = df["close"].values; X = df["adx_14"].values
        for s in STRATS:
            for x in get_strategy(s)(df, {}):
                ...
                r, xb, forced = book_climax(O, H, L, C, X, x.index, x.direction,
                                            entry, stop_d, cost_r, n, ceiling)
...
async def main():
    ...
    for arm, ceiling in ARMS.items():
        sig = signals(frames, _min_stop_atr_multiple(), meta, ceiling)
        ...
        print(f"{arm:<10} trades {len(taken)}, USD/trade {np.mean([t['usd'] for t in taken]):+.4f}, "
              f"climax closes {len(forced)}"
              + (f" (their R {np.mean([t['r'] for t in forced]):+.3f})" if forced else ""), flush=True)
