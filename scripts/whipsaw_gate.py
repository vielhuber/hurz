"""A whipsaw gate: no entry shortly after a signal in the other direction.

Section 280 closed positions on an opposite signal and lost. The entry
side was never read: when the same instrument fired a gated signal the
other way within the last day, the new breakout comes out of a market
that has just broken both ways, and the cooldown of section 283 only
covers stop-outs of positions actually held.

  live       every gated signal
  candidate  refuse a signal when any live strategy fired a gated signal
             in the opposite direction on the same instrument within the
             24 bars before it
  diag       the same within 48 bars
...
def signals(frames, atr_floor, meta, window):
    out = []
    for pair, df in frames.items():
        n = len(df); ts = df["timestamp"].values
        O = df["open"].values; H = df["high"].values
        L = df["low"].values; C = df["close"].values
        raw = [(s, x) for s in STRATS for x in get_strategy(s)(df, {})
               if not gate(s, df, x.index).blocked and not direction_blocked(pair, x.direction)]
        last = {1: np.full(n, -10**9), -1: np.full(n, -10**9)}
        for _, x in raw: last[x.direction][x.index] = x.index
        for d_ in last: last[d_] = np.maximum.accumulate(last[d_])
        for s, x in raw:
            if window and x.index > 0 and x.index - last[-x.direction][x.index - 1] <= window: continue
...
async def main():
    ...
    for arm, window in ARMS.items():
        sig = signals(frames, atr_floor, meta, window)
