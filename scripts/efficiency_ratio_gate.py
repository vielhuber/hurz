"""Kaufman's efficiency ratio as a trend gate: refuse breakouts out of noise.

The router gates the trend strategies on ADX (sections 185, 273), and
sections 131, 203 and 262 read volatility, the broken level's age and the
range before the break. None of them measures how directly price has
travelled: the efficiency ratio over the last 24 bars — net move divided
by the sum of the bar-to-bar moves — is near 1 in a clean trend and near
0 in a market that goes back and forth. A breakout out of a noisy day is
the kind the stop meets first.

  live       every gated signal
  candidate  refuse a signal when the efficiency ratio of the 24 closes
             up to the signal bar is below 0.2
  diag       the same below 0.3
...
ARMS = {"live": None, "candidate": 0.2, "diag": 0.3}
STEP = np.timedelta64(7, 'D')


def noisy(C, e, floor, span=24):
    if floor is None or e < span: return False
    path = float(np.abs(np.diff(C[e - span:e + 1])).sum())
    return path > 0 and abs(float(C[e]) - float(C[e - span])) / path < floor


def signals(frames, atr_floor, meta, floor):
    out = []
    for pair, df in frames.items():
        ...
        for s in STRATS:
            for x in get_strategy(s)(df, {}):
                ...
                if noisy(C, x.index, floor): continue
...
async def main():
    ...
    for arm, floor in ARMS.items():
        sig = signals(frames, atr_floor, meta, floor)
