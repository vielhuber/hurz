"""The momentum strategy at the live 2-ATR stop on the cost-charging walk-forward simulator.

Signals, router-passed signals and count-matched random entries on
twelve instruments. DAYS_FROM / DAYS_TO select the history window. See
docs/EDGE_FINDINGS.md section 73.
"""
...
PAIRS = ["BTCUSD","ETHUSD","OIL_CRUDE","OIL_BRENT","GOLD","DE40","US500","US30","EURUSD","AUDUSD","US100","EU50"]
STRATS=["momentum"]
...
async def main():
    ...
    try:
        ...
        await p.disconnect()
    print(f"\n===== MOMENTUM AT 2-ATR STOP ({DAYS_FROM}-{DAYS_TO} d) =====")
