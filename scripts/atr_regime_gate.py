"""Volatility regime at a 1h signal: ATR% rank against the instrument's own month.

The router gates trend entries on ADX only; how volatile the instrument
is at the moment a breakout fires, relative to its own recent history,
had never been read on the live book (the old vol_squeeze strategy of
section 7 is a different question). Replays the three live 1h trend
strategies on the router-passed path at the live 2-ATR stop over the
whole tradeable universe and buckets every trade by the percentile rank
of ATR(14)/close at the signal bar against the 720 bars (30 days) before
it. Bucket edges are fixed on the recent year and applied unchanged to
the older sample (EDGES env, comma separated). DAYS_FROM / DAYS_TO select
the history window; DUMP writes the raw (r, feature) pairs to a file.
See docs/EDGE_FINDINGS.md section 172.
"""
...
def atr_rank(df):
    """Percentile rank of ATR(14)/close against the 720 bars before each bar."""
    a=(df["atr_14"]/df["close"]).values.astype(float)
    out=np.full(len(a),np.nan)
    for i in range(LOOKBACK,len(a)):
        if not np.isfinite(a[i]): continue
        w=a[i-LOOKBACK:i]; w=w[np.isfinite(w)]
        if len(w)<LOOKBACK*0.9: continue
        out[i]=100.0*np.mean(w<a[i])
    return out

def run(df, entries, pair):
    out=[]; in_until=-1; fee=_fee_for(PLAT,pair)
    O=df["open"].values; H=df["high"].values; L=df["low"].values; C=df["close"].values
    A=df["atr_14"].values; X=df["atr_rank"].values
...
def table(name, tr, edges):
    ...
    print(f"\n--- {name}: n={len(r)} E[R]={r.mean():+.4f} ATR rank median={np.median(feat):.1f} share>=80={np.mean(feat>=80)*100:.0f}%")
    print(f"{'bucket (ATR rank)':<24}{'n':>6}{'E[R]':>9}{'t':>7}{'rest E[R]':>11}{'diff':>9}{'t_diff':>8}")
    row("rank < 20 (low)", feat<20, r)
    row("rank >= 80 (high)", feat>=80, r)
    row("rank >= 95 (extreme)", feat>=95, r)
...
async def main():
    ...
    try:
        for pair in PAIRS:
            ...
            df=add_indicators(_bars_to_df(bars)); df["atr_rank"]=atr_rank(df); n=len(df); seg=n//SEG; per[pair]=[]
            for s in STRATS:
                st=get_strategy(s)
                for k in range(SEG):
    ...
    print(f"\n===== ATR% RANK AT THE 1H SIGNAL, router-passed, 2-ATR stop ({DAYS_FROM}-{DAYS_TO} d) =====")
    print("edges (ATR rank):", ",".join(f"{e:.3f}" for e in edges))
