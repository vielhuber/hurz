"""The remaining active instruments on the cost-charging walk-forward simulator.

Each instrument against count-matched random entries, with the
router-passed subset that live actually trades. DAYS_FROM / DAYS_TO
select the history window. See docs/EDGE_FINDINGS.md section 70.
"""
...
GROUPS={"others":["FR40","UK100","EU50","US100","USDCHF","AUDNZD","EURAUD","HK50","SILVER","AU200","GBPUSD","NZDUSD","J225","COPPER","GBPCAD","AUDJPY","CHFJPY"]}
...
async def main():
    ...
    try:
        ...
        await p.disconnect()
    print(f"\n===== REMAINING ACTIVE INSTRUMENTS at 2-ATR stop, with router-passed subset ({DAYS_FROM}-{DAYS_TO} d) =====")
