"""Trend alignment at a 1h signal: the breakout against its own 200-bar EMA.

The router gates trend entries on ADX strength; whether the breakout
points the same way as the instrument's longer trend had never been
read. Replays the three live 1h trend strategies on the router-passed
path at the live 2-ATR stop over the whole tradeable universe and tags
every trade with the signed distance of the signal close to the causal
EMA(200) of 1h closes (about eight trading days), in ATR(14) units and
in the signal's direction: negative means the breakout fires against
the longer trend. The primary split is aligned (>= 0) against
counter-trend (< 0); quartile edges are fixed on the recent year and
applied unchanged to the older sample (EDGES env, comma separated).
DAYS_FROM / DAYS_TO select the history window; DUMP writes the raw
(r, feature) pairs to a file. See docs/EDGE_FINDINGS.md section 173.
"""
...
def ema_long(df):
    """Causal EMA(200) of the close; NaN inside the warmup so early bars carry no feature."""
...
def run(df, entries, pair):
    ...
    A=df["atr_14"].values; E=df["ema_long"].values
    for e,d in sorted(entries):
        if e<=in_until or e>=len(df): continue
        atr=A[e]
        if not np.isfinite(atr) or atr<=0: continue
        if not np.isfinite(E[e]): continue
        feat=d*(float(C[e])-float(E[e]))/atr
...
def table(name, tr, edges):
    ...
    print(f"\n--- {name}: n={len(r)} E[R]={r.mean():+.4f} dist median={np.median(feat):+.2f} ATR  counter-trend share={np.mean(feat<0)*100:.0f}%")
    print(f"{'bucket (close-EMA200, ATR)':<28}{'n':>6}{'E[R]':>9}{'t':>7}{'rest E[R]':>11}{'diff':>9}{'t_diff':>8}")
    row("counter-trend (< 0)", feat<0, r)
    row("deep counter (< -1)", feat<-1, r)
    row("extended (>= 3)", feat>=3, r)
...
async def main():
    ...
    try:
        for pair in PAIRS:
            ...
            df=add_indicators(_bars_to_df(bars)); df["ema_long"]=ema_long(df); n=len(df); seg=n//SEG; per[pair]=[]
            for s in STRATS:
                st=get_strategy(s)
                for k in range(SEG):
    ...
    print(f"\n===== CLOSE vs EMA200 AT THE 1H SIGNAL, router-passed, 2-ATR stop ({DAYS_FROM}-{DAYS_TO} d) =====")
    print("edges (ATR):", ",".join(f"{e:.3f}" for e in edges))
