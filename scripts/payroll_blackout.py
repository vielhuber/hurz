"""The US payrolls release as a blackout: no entry around the first Friday's 13:30 UTC.

Every scheduled filter measured so far reads the calendar (weekday, turn
of month, holidays, rollover hour, session gaps), never the macro
calendar. The US employment report lands on the first Friday of most
months at 8:30 New York time — 12:30 or 13:30 UTC — and moves FX,
indices, metals and crypto at once. A breakout printed into that release
is a reaction to one number, not the start of a trend, and the stop is
exposed to the spike that follows.

  live       every gated signal
  candidate  refuse signals on the first Friday of the month whose bar
             opens between 12:00 and 15:59 UTC
  diag       refuse every signal on the first Friday of the month

The release date is approximated by the first Friday; months where the
report moved (holidays, shutdowns) are not corrected, which biases the
candidate towards no effect. The stop, the target, the leash, the sizes
and every cap are unchanged; the candidate can only refuse entries.
...
ARMS = {"live": None, "candidate": [12, 13, 14, 15], "diag": list(range(24))}
STEP = np.timedelta64(7, 'D')


def payroll_window(ts, hours):
    t = pd.DatetimeIndex(ts)
    first_friday = (t.dayofweek == 4) & (t.day <= 7)
    return first_friday & np.isin(t.hour, hours)


def signals(frames, atr_floor, meta, hours):
    out = []
    for pair, df in frames.items():
        n = len(df); ts = df["timestamp"].values
        O = df["open"].values; H = df["high"].values
        L = df["low"].values; C = df["close"].values
        blackout = payroll_window(ts, hours) if hours else None
        for s in STRATS:
...
async def main():
    ...
    for arm, hours in ARMS.items():
        sig = signals(frames, atr_floor, meta, hours)
