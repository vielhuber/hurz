"""Failed-breakout reversal on the cost-charging walk-forward simulator.

When a close returns inside the broken level within K bars of a breakout
signal, enter against the breakout at that close; count-matched random
entries serve as the control. DAYS_FROM / DAYS_TO select the history
window. See docs/EDGE_FINDINGS.md section 54.
"""
...
async def main():
    ...
    try:
        ...
        await p.disconnect()
    print(f"\n===== FAILED-BREAKOUT REVERSAL ({DAYS_FROM}-{DAYS_TO} d) =====")
