"""Reward:risk sweep at the live 2-ATR stop on the cost-charging walk-forward simulator.

RR 1.0 / 1.5 / 2.0 / 3.0 for the three live trend strategies pooled, net
result split into gross and cost. DAYS_FROM / DAYS_TO select the history
window. See docs/EDGE_FINDINGS.md section 61.
"""
...
STOPS = [1.0, 1.5, 2.0, 3.0]  # reward:risk values; stop fixed at 2.0 ATR
TO={x:0 for x in STOPS}
STRATS = ["donchian_breakout","turtle_breakout","keltner_breakout"]
DAYS_FROM=int(os.getenv("DAYS_FROM","365")); DAYS_TO=int(os.getenv("DAYS_TO","0"))
SEG=3; HOLD=24; STOP_FIXED=2.0; PLAT="capital_com"

def run_trades(df, entries, pair, RR):
    STOP_ATR=STOP_FIXED
    ...
    for e,d in sorted(entries):
        ...
        if r is None and e+HOLD<len(df): r=(float(C[e+HOLD])-entry)*d/stop_d-cost_r; in_until=e+HOLD; TO[RR]+=1
...
async def main():
    ...
    try:
        ...
        await p.disconnect()
    print(f"\n===== RR SWEEP AT 2-ATR STOP (3 strategies pooled) ({DAYS_FROM}-{DAYS_TO} d) — diff vs rr 1.5 (live), Welch t =====")
    nb,mb,sb=stats(res[1.5])
    print(f"{'rr':>7}{'n':>6}{'E[R]':>9}{'t0':>7}{'cost_r':>8}{'gross':>9}{'diff':>9}{'t_diff':>8}{'g_diff':>8}{'timeout%':>9}")
    gb=mb+costs(res[1.5])
    for P in STOPS:
        n,mu,se=stats(res[P]); d=mu-mb; t=d/np.sqrt(se**2+sb**2) if P!=1.5 else 0.0
