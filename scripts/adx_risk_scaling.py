"""Risk by trend strength: half the risk on the weakest trend signals.

The router admits trend signals at ADX >= 30 and the ceiling refuses the
top (sections 185, 273); inside that band every entry carries the same
3 USD target risk. Sections 185 and 274 read the target and the floor by
ADX, never the size. If the band's lower edge earns less per R than the
rest, moving risk away from it raises the dollars per unit of risk
without touching the 3 USD ceiling any trade may carry.

  live       every entry at the 3 USD target risk
  candidate  entries with ADX(14) below 35 at the signal bar sized at
             1.5 USD target risk, the rest unchanged
  diag       the same at 2.25 USD

Sizes go through `calculate_position_size` with the venue's real size
steps, so a smaller target risk can round a position to nothing and skip
it, as live. No limit is raised; the candidate can only size down. Weekly
...
from app.spot_trading.regime import gate, adx_at
...
ARMS = {"live": None, "candidate": 0.5, "diag": 0.75}
WEAK_ADX = 35.0
FULL_RISK = ews.DEFAULT_TARGET_RISK_USD
STEP = np.timedelta64(7, 'D')


def signals(frames, atr_floor, meta, scale):
    out = []
    for pair, df in frames.items():
        ...
        for s in STRATS:
            for x in get_strategy(s)(df, {}):
                ...
                adx = adx_at(df, x.index)
                weak = scale is not None and adx is not None and adx < WEAK_ADX
                ews.DEFAULT_TARGET_RISK_USD = FULL_RISK * scale if weak else FULL_RISK
                terms = trade_terms(df, x.index, pair, meta, atr_floor)
                ews.DEFAULT_TARGET_RISK_USD = FULL_RISK
...
async def main():
    ...
    for arm, scale in ARMS.items():
        sig = signals(frames, atr_floor, meta, scale)
        ...
        print(f"{arm:<10} signals {len(sig)}, trades {len(booked)}, "
              ...
              f"risk/trade {np.mean([t['risk'] for t in booked]):.3f}", flush=True)
