"""GOLD stop treatment on the cost-charging walk-forward simulator.

Raw 2-ATR stop at GOLD's 0.1 percent venue minimum, widened to the shared
1.05 percent minimum as the backtests do, and refused under the 1 percent
floor as live did. DAYS_FROM / DAYS_TO select the history window. See
docs/EDGE_FINDINGS.md section 104.
"""
...
def run_trades(df, entries, pair, MODE):
    ...
    for e,d in sorted(entries):
        ...
        if MODE=='widened':
            ...
            if stop_d<0.0105*entry:
                if stop_d/entry<0.01: continue
        else:
            vm=0.00105*entry
            if stop_d<vm: stop_d=vm
        ...
        if r is None and e+HOLD<len(df): r=(float(C[e+HOLD])-entry)*d/stop_d-cost_r; in_until=e+HOLD; TO[MODE]+=1
...
async def main():
    ...
    try:
        ...
        await p.disconnect()
    print(f"\n===== GOLD STOP TREATMENT ({DAYS_FROM}-{DAYS_TO} d) =====")
    for m in STOPS:
        a=np.array([x[0] for x in res[m]]); c=np.array([x[1] for x in res[m]])
        if len(a)==0: print(f"{m:<8} n=0"); continue
        se=a.std(ddof=1)/np.sqrt(len(a))
        print(f"{m:<8} n={len(a):>4} E[R]={a.mean():+.4f} t={a.mean()/se:+.2f} cost_r={c.mean():.4f} gross={(a+c).mean():+.4f} sumR={a.sum():+.1f} timeouts={TO[m]}")
asyncio.run(main())
