"""All measured instruments on the cost-charging walk-forward simulator.

Each instrument against count-matched random entries, with the
router-passed subset, for a configurable history window (DAYS_FROM /
DAYS_TO). Used for the year-to-year persistence check in
docs/EDGE_FINDINGS.md section 97.
"""
...
GROUPS={"all":["BTCUSD","ETHUSD","GOLD","OIL_CRUDE","OIL_BRENT","DE40","US500","US30","EURUSD","AUDUSD","FR40","UK100","EU50","US100","USDCHF","AUDNZD","EURAUD","HK50","SILVER","GBPUSD","NZDUSD","J225","COPPER","GBPCAD","AUDJPY","CHFJPY","EURJPY","GBPJPY","CADJPY","USDJPY","AU200"]}
...
async def main():
    ...
    res={g:{"sig":[],"rand":[],"passed":[]} for g in GROUPS}; per={}
    try:
        for g,pairs in GROUPS.items():
            for pair in pairs:
                ...
                df=add_indicators(_bars_to_df(bars)); n=len(df); seg=n//SEG; per[pair]={"sig":[],"rand":[],"passed":[]}
                for s in STRATS:
                    st=get_strategy(s)
                    for k in range(SEG):
                        ...
                        ps=run(sdf,[(x.index,x.direction) for x in st(sdf,{}) if not gate(s,sdf,x.index).blocked],pair); per[pair]["passed"].extend(ps); res[g]["passed"].extend(ps)
                        for _ in range(3):
        ...
        await p.disconnect()
    print(f"\n===== ALL 31 INSTRUMENTS at 2-ATR stop, with router-passed subset ({DAYS_FROM}-{DAYS_TO} d) =====")
    print(f"{'group/pair':<12}{'n':>6}{'E_sig':>9}{'t0':>7}{'E_rand':>9}{'sig-rand':>10}{'t':>7}{'pinned%':>9}{'n_pass':>7}{'E_pass':>9}{'t_pass':>7}")
    def row(name,d):
        s=[x[0] for x in d["sig"]]; r=[x[0] for x in d["rand"]]; ns,ms,ss=stats(s); nr,mr,sr=stats(r)
        pp=[x[0] for x in d["passed"]]; npp,mpp,spp=stats(pp) if len(pp)>1 else (len(pp),float('nan'),float('nan'))
        print(f"{name:<12}{ns:>6}{ms:>+9.4f}{ms/ss:>+7.2f}{mr:>+9.4f}{ms-mr:>+10.4f}{(ms-mr)/np.sqrt(ss**2+sr**2):>+7.2f}{np.mean([x[1] for x in d['sig']])*100:>9.0f}{npp:>7}{mpp:>+9.4f}{mpp/spp if npp>1 else float('nan'):>+7.2f}")
    for g in GROUPS:
        row(g.upper(),res[g])
