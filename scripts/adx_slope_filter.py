"""ADX slope at the signal bar: rising versus fading trend strength.
...
trade by the change of ADX(14) over the three bars before the signal
(adx[e] - adx[e-3]). The router already requires ADX >= 30; this asks
whether the direction of ADX at that moment carries information. Bucket
edges are fixed on the recent year and applied unchanged to the older
sample (EDGES env, comma separated). DAYS_FROM / DAYS_TO select the
history window. See docs/EDGE_FINDINGS.md section 112.
"""
...
SEG=3; RR=1.5; STOP_ATR=2.0; HOLD=24; PLAT="capital_com"; SLOPE_BARS=3
...
def run(df, entries, pair):
    ...
    A=df["atr_14"].values; X=df["adx_14"].values
    for e,d in sorted(entries):
        if e<=in_until or e>=len(df) or e<SLOPE_BARS: continue
        atr=A[e]
        if not np.isfinite(atr) or atr<=0: continue
        feat=X[e]-X[e-SLOPE_BARS]
...
def table(name, tr, edges):
    ...
    print(f"\n--- {name}: n={len(r)} E[R]={r.mean():+.4f} slope median={np.median(feat):+.2f} rising={np.mean(feat>=0)*100:.0f}%")
    print(f"{'bucket':<24}{'n':>6}{'E[R]':>9}{'t':>7}{'rest E[R]':>11}{'diff':>9}{'t_diff':>8}")
    row("falling (slope < 0)", feat<0, r)
    row("rising (slope >= 0)", feat>=0, r)
...
async def main():
    ...
    print(f"\n===== ADX SLOPE ({SLOPE_BARS} bars), router-passed, 2-ATR stop ({DAYS_FROM}-{DAYS_TO} d) =====")
    print("edges (ADX change):", ",".join(f"{e:.3f}" for e in edges))
