"""Holding-leash sweep at the live 2-ATR stop on the cost-charging walk-forward simulator.

Holds 12 / 24 / 48 / 96 for the three live trend strategies pooled, net
result split into gross and cost. DAYS_FROM / DAYS_TO select the history
window. See docs/EDGE_FINDINGS.md section 62.
"""
...
STOPS = [12, 24, 48, 96]  # max_hold values; stop fixed at 2.0 ATR, rr 1.5
TO={x:0 for x in STOPS}
STRATS = ["donchian_breakout","turtle_breakout","keltner_breakout"]
DAYS_FROM=int(os.getenv("DAYS_FROM","365")); DAYS_TO=int(os.getenv("DAYS_TO","0"))
SEG=3; STOP_FIXED=2.0; RR_FIXED=1.5; PLAT="capital_com"

def run_trades(df, entries, pair, HOLD):
    STOP_ATR=STOP_FIXED; RR=RR_FIXED
    ...
    for e,d in sorted(entries):
        ...
        if r is None and e+HOLD<len(df): r=(float(C[e+HOLD])-entry)*d/stop_d-cost_r; in_until=e+HOLD; TO[HOLD]+=1
...
async def main():
    ...
    try:
        ...
        await p.disconnect()
    print(f"\n===== MAX_HOLD SWEEP AT 2-ATR STOP (3 strategies pooled) ({DAYS_FROM}-{DAYS_TO} d) — diff vs hold 24 (live), Welch t =====")
    nb,mb,sb=stats(res[24])
    print(f"{'hold':>7}{'n':>6}{'E[R]':>9}{'t0':>7}{'cost_r':>8}{'gross':>9}{'diff':>9}{'t_diff':>8}{'g_diff':>8}{'timeout%':>9}")
    gb=mb+costs(res[24])
    for P in STOPS:
        n,mu,se=stats(res[P]); d=mu-mb; t=d/np.sqrt(se**2+sb**2) if P!=24 else 0.0
