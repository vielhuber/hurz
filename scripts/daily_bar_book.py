"""The same breakout strategies on daily bars, beside the hourly book.

Every measurement in this document trades the 1h bar. The 15m book was
worse (section 29), the 4h variants dead (run 24), the 2h book flat
(section 246) — all of them shorter or adjacent. The daily bar, the
timeframe the turtle rules were written for, has never been booked here,
and section 297 has just shown that what this book earns comes from the
few trades that run rather than from turning slots over faster.

  daily bars  1h bars resampled to UTC days, indicators recomputed
  stop        3 x ATR(14) of the daily bar, the floor `HURZ_MIN_STOP_ATR
              _MULTIPLE` demands, or the venue minimum if that is wider
  target      1.5 R as live; leash 24 daily bars
  ...
  candidate   1h and 1d combinations ranked together, same list of 40,
              same caps, one position per instrument across both
  diag        the 1d book alone

Sizes, caps, cooldown, pins and vetoes are the live ones; the daily
...
def daily_frames(frames):
    ...
    for pair, df in frames.items():
        ...
        d = d.set_index(pd.DatetimeIndex(d["timestamp"])).resample("1D").agg(
        ...
        for s in STRATS:
            for x in get_strategy(s)(df, {}):
                ...
                out.append({"ts": ts[x.index], "exit_ts": ts[xb], "pair": pair,
                            "dir": x.direction, "strat": s + "_1d", "r": r,
...
async def main():
    ...
    print(f"hourly signals {len(hourly)}, daily signals {len(daily)} on {len(dframes)} instruments, "
    ...
    for arm in ARMS:
        ...
        share = sum(1 for t in booked if t["strat"].endswith("_1d"))
        print(f"{arm:<10} trades {len(booked)} ({share} daily), "
