"""Bank index profits at the US cash close: exit at 20:00 UTC when the open result is large enough.

The index positions hold through the leash of 24 bars, so nearly every
one sits through a cash close and the overnight session, where the
indices trade thin and gap at the next open. Section 276 found no
overnight drift worth holding for, and section 297 found that cutting
positions early on their shape costs. Neither asked whether a profit that
exists at the cash close should be taken there instead of carried into
the night.

  live       stop, target at 1.5 R, leash at 24 bars
  candidate  the same; a position on US500, US30, US100, DE40, EU50,
             FR40, UK100, J225 or HK50 is closed at the close of the bar
             opening 19:00 UTC (20:00 UTC) if its open result there is at
             least +0.5 R
  diag       the same at any open result above zero

Non-index positions, the stop, the target, the sizes and every cap are
unchanged; the candidate can only close positions earlier. Weekly
...
ARMS = {"live": None, "candidate": 0.5, "diag": 1e-9}
...
def book(O, H, L, C, e, d, entry, stop_d, cost_r, n, bank, hours):
    """`efficiency_weighted_selection.book` plus the cash-close profit exit."""
    ...
    for b in range(e + 1, e + HOLD + 1):
        ...
        if bank is not None and hours[b] == 19:
            open_r = (float(C[b]) - entry) * d / stop_d
            if open_r >= bank: return open_r - cost_r, b
...
def signals(frames, atr_floor, meta, stale):
    ...
    for pair, df in frames.items():
        ...
        bank = stale if pair in INDICES else None
        for s in STRATS:
            for x in get_strategy(s)(df, {}):
                ...
                r, xb = book(O, H, L, C, x.index, x.direction, entry, stop_d, cost_r, n, bank, hours)
