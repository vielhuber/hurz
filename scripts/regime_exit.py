"""ADX regime exit on the cost-charging walk-forward simulator.

Close a trade at the first bar whose ADX(14) drops below a threshold,
against the fixed 24-bar leash at the live 2-ATR stop. DAYS_FROM /
DAYS_TO select the history window. See docs/EDGE_FINDINGS.md section 69.
"""
...
STOPS = [0, 15, 20, 25]  # ADX exit thresholds; 0 = live (no regime exit)
TO={x:0 for x in STOPS}
STRATS = ["donchian_breakout","turtle_breakout","keltner_breakout"]
DAYS_FROM=int(os.getenv("DAYS_FROM","365")); DAYS_TO=int(os.getenv("DAYS_TO","0"))
SEG=3; RR=1.5; HOLD=24; STOP_FIXED=2.0; PLAT="capital_com"

def run_trades(df, entries, pair, ADX_EXIT):
    STOP_ATR=STOP_FIXED; ADX=df["adx_14"].values
    ...
    for e,d in sorted(entries):
        ...
        for b in range(e+1,e+HOLD+1):
            ...
            if ADX_EXIT and np.isfinite(ADX[b]) and ADX[b]<ADX_EXIT: r=(float(C[b])-entry)*d/stop_d-cost_r; in_until=b; TO[ADX_EXIT]+=1; break
        if r is None and e+HOLD<len(df): r=(float(C[e+HOLD])-entry)*d/stop_d-cost_r; in_until=e+HOLD; TO[0]+=1
...
async def main():
    ...
    try:
        ...
        await p.disconnect()
    print(f"\n===== ADX REGIME EXIT (3 strategies pooled) ({DAYS_FROM}-{DAYS_TO} d) — diff vs no regime exit (live), Welch t =====")
    nb,mb,sb=stats(res[0])
    print(f"{'adx<':>7}{'n':>6}{'E[R]':>9}{'t0':>7}{'cost_r':>8}{'gross':>9}{'diff':>9}{'t_diff':>8}{'g_diff':>8}{'early%':>9}")
    gb=mb+costs(res[0])
    for P in STOPS:
        n,mu,se=stats(res[P]); d=mu-mb; t=d/np.sqrt(se**2+sb**2) if P!=0 else 0.0
        c=costs(res[P]); g=mu+c
        print(f"{P:>7}{n:>6}{mu:>+9.4f}{mu/se:>+7.2f}{c:>8.4f}{g:>+9.4f}{d:>+9.4f}{t:>+8.2f}{g-gb:>+8.4f}{TO[P]/n*100:>9.1f}")
asyncio.run(main())
