"""Room to the target: refuse a breakout whose target lies beyond an older extreme.

Every live strategy enters on the break of a short channel (20 to 55 bars)
and aims at 1.5 R. None of the gates reads what stands between the entry
and that target: a long taken under the high of the last ten days has to
clear that high before it can book the target, and a level that turned
price once is where a short breakout is most likely to stall. Section 203
split the signals by the age of the broken level and 262 by the width of
the range before it; neither asked whether the target is reachable
without crossing an older extreme.

  live       every gated signal
  candidate  refuse a long when the highest high of the 240 bars before
             the signal bar lies above the entry and below the target
             (a short: the lowest low above the target and below entry)
  diag       the same with 480 bars

The stop, the target, the leash, the sizes and every cap are unchanged;
the candidate can only refuse entries. Weekly re-ranking, open positions
...
ARMS = {"live": None, "candidate": 240, "diag": 480}
STEP = np.timedelta64(7, 'D')


def blocked_by_room(H, L, e, d, entry, stop_d, lookback):
    if lookback is None or e < lookback: return False
    tp = entry + d * RR * stop_d
    if d == 1:
        level = float(H[e - lookback:e].max())
        return entry < level < tp
    level = float(L[e - lookback:e].min())
    return tp < level < entry


def signals(frames, atr_floor, meta, lookback):
    out = []
    for pair, df in frames.items():
        ...
        for s in STRATS:
            for x in get_strategy(s)(df, {}):
                ...
                if blocked_by_room(H, L, x.index, x.direction, entry, stop_d, lookback): continue
...
async def main():
    ...
    for arm, lookback in ARMS.items():
        sig = signals(frames, atr_floor, meta, lookback)
