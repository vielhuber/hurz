"""Refusing breakouts in a calm market: the market-wide volatility regime.

Trend following earns its money in markets that move: a breakout carries
when volatility across assets is high and dissolves into noise when every
market is quiet. The regime filters so far read the instrument's own
volatility against its last month (run 57), the direction of the market's
breadth (the third run of 2026-09-10) and trend strength by ADX; none read
how much the market as a whole is moving.

  vol rank   per instrument, the percentile rank of ATR(14)/close against
             its 2,160 bars (90 days) before, carried forward at most 72
             bars
  market     the median rank across all instruments at the signal bar
  live       as today
  candidate  refused if the market's median rank is below 0.25
  diag       refused if the market's median rank is below 0.40
...
ARMS = {"live": None, "candidate": 0.25, "diag": 0.40}
RANK_BARS = 2160
...
async def main():
    ...
    for pair, df in frames.items():
        atr_pct = pd.Series(df["atr_14"].values / df["close"].values, index=pd.DatetimeIndex(df["timestamp"].values))
        ranks[pair] = atr_pct.rolling(RANK_BARS + 1, min_periods=RANK_BARS + 1).rank(pct=True)
    market = pd.concat(ranks, axis=1).sort_index().ffill(limit=72).median(axis=1, skipna=True)
    m_ts = market.index.values; m_v = market.values
    for s in base:
        i = int(np.searchsorted(m_ts, s["ts"], side="right")) - 1
        s["mkt"] = float(m_v[i]) if i >= 0 and np.isfinite(m_v[i]) else float("nan")
    mkt = np.array([s["mkt"] for s in base])
    known = np.isfinite(mkt)
    print(f"signals {len(base)}, with a market rank {known.sum()}, quartiles "
          f"{np.percentile(mkt[known], 25):.3f} / {np.median(mkt[known]):.3f} / {np.percentile(mkt[known], 75):.3f}, "
          f"below 0.25: {(mkt[known] < 0.25).mean():.1%}, below 0.40: {(mkt[known] < 0.40).mean():.1%}", flush=True)
    now = np.datetime64(datetime.now(timezone.utc).date())
    rank_w = np.timedelta64(RANK_DAYS, 'D')

    results = {}
    for arm, floor in ARMS.items():
        sig = base if floor is None else [s for s in base if not s["mkt"] < floor]
        ...
        if floor is None:
            for f in (0.25, 0.40):
                low = np.array([t["usd"] for t in booked if t["mkt"] < f])
