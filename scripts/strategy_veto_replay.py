"""Strategy-level veto replay on the cost-charging simulator.

Retires a whole strategy once its running mean R over at least N trades
is at or below a threshold, for the live rule and two variants, and
measures what the retired trades would have returned. DAYS_FROM /
DAYS_TO select the history window. See docs/EDGE_FINDINGS.md section 99.
"""
...
STOPS = [2.0]
RULES=[(25,-0.10),(50,-0.10),(100,-0.05)]
...
async def main():
    ...
    print(f"\n===== STRATEGY VETO REPLAY per strategy ({DAYS_FROM}-{DAYS_TO} d) =====")
    ...
    for mn,thr in RULES:
        ...
        print(f"rule >= {mn:>3} trades & mean <= {thr:+.2f}: retired strategies={sorted(retired)} | blocked n={nb:>5} E[R]_blocked={mb:+.4f} t={mb/sb if nb>1 else 0:+.2f} | kept n={len(kept)} E[R]_kept={np.mean(kept):+.4f} | delta vs no veto={sum(kept)-base:+.1f}")
asyncio.run(main())
