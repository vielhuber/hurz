"""An upper ADX bound on trend entries: the band instead of the floor.

Section 186 set out to explain section 185's flipped target and found
something else in the column beside it. The live rule's expectancy
falls monotonically with ADX at entry on the recent year — -0.008 R in
the 30-35 bucket down to -0.144 R above 50 — while on the older sample
it carries no ADX ordering at all (+0.018 to -0.016 R). The regime
router gates trend entries at ADX >= 30 and has no upper bound. This
run asks whether one belongs there.

The hypothesis comes from data already seen, so the acceptance rule is
stated in full before the measurement and is deliberately not the
symmetric t > 2 on both samples used for rule changes. A filter
removes trades rather than adding exposure, and a filter that helps
strongly in one regime and is neutral in the other can never show
t > 2 twice — demanding it would reject every conditional exclusion by
construction. Preregistered instead:

  (a) the paired difference is positive in the point estimate on BOTH
      walk-forward samples,
  (b) it reaches t > 2 on at least one,
  (c) it does not fall below t = -1 on either,
  (d) the sign is stable across the threshold sweep rather than
      appearing at a single cherry-picked value,
  (e) the live journal does not contradict it.

Failing any of these, nothing is built. Occupancy follows the live
rule, so a filtered trade does not free its slot for a later signal —
the measurement understates the filter and never flatters it. Every
signal's live R is set against the variant's R, zero where the variant
does not trade, and compared paired. DAYS_FROM / DAYS_TO select the
window. See docs/EDGE_FINDINGS.md section 187.
"""
...
def run(df, entries, pair):
    ...
    for e,d in sorted(entries):
        ...
        in_until=xb
        out.append((float(adx), r))
    return out

def report(name, tr):
    if len(tr)<4: return
    adx=np.array([a for a,_ in tr]); live=np.array([r for _,r in tr])
    t=lambda v: v.mean()/(v.std(ddof=1)/np.sqrt(len(v))) if len(v)>1 and v.std(ddof=1)>0 else float('nan')
    print(f"\n--- {name}: n={len(tr)} live E[R]={live.mean():+.4f} sumR={live.sum():+.1f} t={t(live):+.2f}")
    print(f"{'ceiling':<12}{'kept':>7}{'cut':>6}{'cut E[R]':>11}{'rule E[R]':>11}{'diff':>10}{'t_pair':>8}{'rule sumR':>11}")
    for c in CEILINGS:
        keep=adx<c; cut=~keep
        if cut.sum()<10: continue
        rule=np.where(keep, live, 0.0); d=rule-live
        print(f"{f'ADX<{c:g}':<12}{int(keep.sum()):>7}{int(cut.sum()):>6}{live[cut].mean():>+11.4f}"
...
async def main():
    ...
    try:
        ...
        await p.disconnect()
    print(f"\n===== ADX CEILING ON TREND ENTRIES ({DAYS_FROM}-{DAYS_TO} d) =====")
    report("ALL", allt)
