"""Hysteresis on the active list: a combination that drops out stays for four more weeks.

The selector rebuilds the list every week from the trailing year and a
combination leaves it the week it falls below the eligibility rule (10
trades, pf >= 0.8, eR >= -0.2). Section 317 found the list fragile: a
blackout that removed 115 index trades in seven years cost a third of the
book's dollars, because the missing trades pushed whole combinations off
the list. Sections 217 and 301 found the eligibility thresholds on a
plateau, so a combination at the edge is not measurably worse than one
inside; dropping it on a single week's reading throws away its trades.

  live       the list is this week's ranking
  candidate  a combination that was on the list in any of the last four
             weekly rankings stays on it, even if this week's ranking
             drops it
  diag       the same over the last eight weekly rankings

Pins, vetoes, sizes and every cap are unchanged; the list can only grow
by combinations that were eligible within the window. Weekly re-ranking,
open positions and cooldowns carried across ranking boundaries (section
284). Run from the bot's checkout (section 286).

Acceptance, fixed before the data were seen:
  (a) USD per calendar day better on ALL FOUR year-samples,
  (b) pooled paired t > +2.

See docs/EDGE_FINDINGS.md 318.
"""
...
ARMS = {"live": 1, "candidate": 5, "diag": 9}
...
async def main():
    ...
    for arm, keep in ARMS.items():
        ...
        while cut < end:
            ...
            history = (history + [ranked])[-keep:]
            active = set().union(*history)
            admit(sig[hi:int(np.searchsorted(ts_all, nxt))], active | pins, "live", None, state, booked, [])
            cut = nxt
        daily = {}
        for t in booked:
        ...
        results[arm] = daily
        print(f"{arm:<10} list {len(active)}, trades {len(booked)}, "
