"""Concurrent-position cap replay on the one-position-per-instrument timeline.

Refuses entries while N positions are open, at caps of 4 / 6 / 8 / 12, on
the cost-charging simulator at the live 2-ATR stop. DAYS_FROM / DAYS_TO
select the history window. See docs/EDGE_FINDINGS.md section 85.
"""
...
async def main():
    ...
    import heapq
    print(f"\n===== CONCURRENT-POSITION CAP REPLAY, ONE POSITION PER INSTRUMENT ({DAYS_FROM}-{DAYS_TO} d) =====")
    base=sum(t[0] for t in trades); print(f"no cap: n={len(trades)} sumR={base:+.1f} E[R]={base/len(trades):+.4f}")
    # peak concurrency without a cap
    ev=sorted([(t[2],1) for t in trades]+[(t[3],-1) for t in trades]); cur=0; peak=0; hist={}
    for _,d in ev:
        cur+=d; peak=max(peak,cur)
    print(f"peak simultaneous positions without a cap: {peak}")
    for cap in CAPS:
        open_exits=[]; kept=[]; blocked=[]
        for r,c,et,xt in trades:
            while open_exits and open_exits[0] <= et: heapq.heappop(open_exits)
            if len(open_exits) >= cap: blocked.append(r); continue
            kept.append(r); heapq.heappush(open_exits, xt)
        print(f"cap {cap:>2}: kept n={len(kept)} sumR={sum(kept):+.1f} | blocked n={len(blocked)} ({len(blocked)/len(trades)*100:.1f}%) E[R]_blocked={(sum(blocked)/len(blocked)) if blocked else 0:+.4f} E[R]_kept={sum(kept)/len(kept):+.4f} | delta vs no cap={sum(kept)-base:+.1f}")
asyncio.run(main())
