...
## 200. Two consequences of today's builds: a contaminated scaling sample, and a metric that can show the effect

Sections 188, 190 and 192 changed what the system trades. Two things
elsewhere in the project were left inconsistent with that, and both are
corrected here rather than measured — they are bookkeeping, not levers,
and are recorded as such.

**The scaling gate's out-of-sample window.** `edge_scaling.py` excludes
trades before its cutoff on the stated grounds that "the entry filters
were calibrated on them, so their expectancy is in-sample and worthless
as evidence for sizing". The cutoff was 2026-08-24. But the three filters
that now trade were calibrated on four walk-forward samples whose most
recent window is the last 365 days — which contains the entire live
journal. Every trade before the builds is therefore in-sample for them,
including the 37 the gate had accumulated. Left alone, the gate would
eventually have scaled risk on evidence generated by a different system.

The cutoff moves to 2026-09-10 and the sample restarts at zero. This
costs nothing real: section 195 established the gate needs 2,750 to 9,858
trades to open at all, so the 37 it had were never close to the
threshold. What it removes is the possibility of opening on contaminated
evidence if expectancy ever ran high enough.

**A metric that can see the change.** The project rule requires the
dashboard to make the daily gain visible, and none of its current figures
can: the 7- and 30-day rates average trading days from before the builds,
and all-time averages 121. With zero entries since the builds — it is a
weekend — the 30-day rate of -0.88 USD/day is arithmetically frozen at a
number describing the previous system.

The card gains one row, keyed to `_FILTER_EPOCH`: realised PnL and per-day
rate over entries accepted since the builds only, and while that set is
empty it says "noch kein Trade" rather than showing a misleading 0.00.
This is the figure that will answer whether the four-sample expectancy
improvement survives forward, and it is now visible from the first trade
onward instead of being diluted for a month.
...
## 261. Adding to a cluster's winners: the refused additions to losers are the better trades

Section 253 found that the positions a full cluster holds longest sit at
+0.24 R — the cluster fills during a broad move and its first members
caught it. Trend following's oldest rule suggested the factor-level
version: admit a further same-direction entry into a cluster only while
the positions already held in that direction are winning.
`scripts/cluster_add_to_winners.py` refuses such an entry when the held
positions' mean open R at the signal bar is not above zero (diagnostic:
when any is not above zero). The first position in a cluster direction is
never affected. Live-faithful book of section 255.

(This run was interrupted by the host reboot of 2026-09-13, which wiped
the bar cache under /tmp; the cache was rebuilt under /var/tmp and the
run repeated unchanged.)

| variant | trades | USD/trade | refused | their USD/signal | pooled USD/day | diff | t | up |
|---|---|---|---|---|---|---|---|---|
| **live** | **5,097** | **+0.0533** | — | — | **+0.1791** | — | — | — |
| mean open R > 0 | 4,263 | +0.0480 | 4,177 | **+0.0822 (t +3.39)** | +0.1348 | **-0.0442** | **-0.94** | **1/4** |
| every one > 0 (diag) | 3,998 | +0.0378 | 6,371 | +0.0405 (t +2.06) | +0.0993 | -0.0793 | -1.60 | 1/4 |

Candidate per sample: +0.0475 / -0.0546 / -0.0239 / -0.0381. Clauses (a)
and (b) fail.

**The hypothesis is reversed, and significantly.** The entries the rule
refuses — additions to a cluster direction whose open positions are under
water — earn **+0.0822 USD a signal at t +3.39**, better than the book's
mean trade. A same-direction breakout arriving while the factor's
positions are losing is, on this evidence, a better entry than one
arriving while they win. That fits section 253 read differently: an
incumbent that is already winning has consumed part of the move, and a
late follower into a winning cluster buys the extension; a breakout
against the grain of losing positions catches the turn.

This is a hypothesis generated by this run, not a result, and it is not
acted on: the statistic is pooled over the same seven years any
walk-forward would score it on. Section 115's rule applies — read it on
data that did not select it. **Preregistered for the next run:** on the
live journal, entries opened while same-cluster same-direction positions
...
## 286. Where the breakout bar closes in its own range: the adverse-half closes are not the bad ones

Section 92 found the channel crossings a close takes back worth -0.05 R
against the confirmed entry. A confirmed breakout can still close far from
its extreme — a long that spiked through the level and closed near its
low. No entry filter had read the close's location inside the signal
bar's range. `scripts/breakout_close_location.py` computes it as
(close − low)/(high − low) for a long, (high − close)/(high − low) for a
short, and refuses signals below 0.5 (candidate) or 0.25 (diagnostic),
before the selector sees them. Weekly re-ranking, carried positions and
cooldowns (section 284), caps, pins and live vetoes as live. Clauses (a)
...
| reverse list order (diag) | 5,110 | +0.0552 | 3,576 (+0.0482, 0.012) | 3,582 (+0.0556, 0.012) | +0.1836 | -0.0175 | -0.63 | 1/4 |
...
| 4h book alone (diag) | 1,358 (1,358) | -0.0910 | -0.0756 | -0.2194 | -2.27 | 0/4 |
